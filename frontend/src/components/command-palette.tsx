...
export function CommandPalette({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const commands: Command[] = [
    ...
    { id: 'logout', label: 'Logout', shortcut: '', icon: LogOut, section: 'Actions', action: () => { localStorage.removeItem('apiKey'); window.location.href = '/login' } },
  ]
        ...
        >
          ...
          >
            <div className="flex items-center gap-3 border-b border-border px-4">
              ...
              />
              <kbd className="hidden shrink-0 rounded-md border border-border px-1.5 py-0.5 text-[10px] text-muted-foreground sm:inline">ESC</kbd>
            </div>

            <div className="max-h-80 overflow-y-auto px-2 py-2">
              {Object.entries(grouped).map(([section, items]) => (
                <div key={section}>
                  <p className="px-2 py-1.5 text-[11px] font-medium uppercase tracking-wider text-muted-foreground">{section}</p>
                  {items.map((cmd, i) => {
              ...
              {filtered.length === 0 && (
                <p className="px-2 py-8 text-center text-sm text-muted-foreground">No results for &ldquo;{query}&rdquo;</p>
              )}
            </div>
